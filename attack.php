...
$result = $db->Execute ("SELECT * FROM {$db->prefix}ships WHERE ship_id='$ship_id'");
...
// Easter egg comment - So I called Clive Dinkey's - the home of the No splinter, full woody, tropical dream vacation and spa salon.
...
else
{
    ...
    $res = $db->Execute("SELECT allow_attack,{$db->prefix}universe.zone_id FROM {$db->prefix}zones,{$db->prefix}universe WHERE sector_id='$targetship[sector_id]' AND {$db->prefix}zones.zone_id={$db->prefix}universe.zone_id");
    $query97 = $res->fields;
    ...
    else
    {
        ...
        if ($targetship['dev_emerwarp'] > 0 && $random_value > $chance)
        {
            ...
            $debug_query = $db->Execute("UPDATE {$db->prefix}players SET turns=turns-1,turns_used=turns_used+1,rating=rating-$rating_change WHERE player_id=$playerinfo[player_id]");
            db_op_result($db,$debug_query,__LINE__,__FILE__);

            playerlog($db,$targetinfo['player_id'], "LOG_ATTACK_EWD", "$playerinfo[character_name]");

            $debug_query = $db->Execute ("UPDATE {$db->prefix}ships SET sector_id=$dest_sector, dev_emerwarp=dev_emerwarp-1,cleared_defenses=' ' WHERE ship_id=$ship_id");
            db_op_result($db,$debug_query,__LINE__,__FILE__);
        ...
        else
        {
            ...
            if ((($targetscore / $playerscore < $bounty_ratio) || $targetinfo['turns_used'] < $bounty_minturns) && !("aiplayer" == substr($targetinfo['email'], -8) ) )
            {
                ...
                $btyamount = 0;
                $hasbounty = $db->Execute("SELECT SUM(amount) AS btytotal FROM {$db->prefix}bounty WHERE bounty_on = $targetinfo[player_id] AND placed_by = 0");
                if ($hasbounty)
                {
...
//                  echo "$temp - $playerfighters - $targettorpdmg"; Im not sure this is supposed to be commented out, but..
            ...
            if ($targetarmor < 1) // FOO - VICDIE
            {
                ...
                if ($playerarmor > 0)
                {
                    ...
                    {
                        $db->Execute("UPDATE {$db->prefix}ai SET active='N' WHERE ai_id=$targetinfo[email]");
                        if ($rating_change > 0)
                        {
                    ...
                    $debug_query = $db->Execute ("UPDATE {$db->prefix}ships SET ore=ore+$salv_ore, organics=organics+$salv_organics, goods=goods+$salv_goods,energy=$energy, fighters=fighters-$fighters_lost,armor_pts=armor_pts-$armor_lost,torps=torps-$playertorpnum WHERE player_id=$playerinfo[player_id] AND ship_id=$playerinfo[currentship]");
                    db_op_result($db,$debug_query,__LINE__,__FILE__);

                    $debug_query = $db->Execute ("UPDATE {$db->prefix}players SET credits=credits+$ship_salvage, turns=turns-1,turns_used=turns_used+1,rating=rating-$rating_change WHERE player_id=$playerinfo[player_id]");
                    db_op_result($db,$debug_query,__LINE__,__FILE__);
            ...
            else
            {
                ...
                $debug_query = $db->Execute ("UPDATE {$db->prefix}ships SET energy=$energy,fighters=fighters-$fighters_lost, armor_pts=armor_pts-$armor_lost, torps=torps-$targettorpnum WHERE ship_id=$ship_id");
                db_op_result($db,$debug_query,__LINE__,__FILE__);
                ...
                $energy=$shipinfo['energy'];
//              FOO - ENERGY BUG HERE?
                $debug_query = $db->Execute ("UPDATE {$db->prefix}ships SET energy=$energy,fighters=fighters-$fighters_lost, armor_pts=armor_pts-$armor_lost, torps=torps-$playertorpnum WHERE player_id=$playerinfo[player_id] AND ship_id=$playerinfo[currentship]");
                db_op_result($db,$debug_query,__LINE__,__FILE__);
...
//              playerdeath($db,$playerinfo['player_id'], "LOG_ATTACK_LOSE", "$playerinfo[character_name]|N", 1, $targetinfo['player_id'], $shipinfo['ship_id']);
                ...
                if ($targetarmor > 0)
                {
                    ...
                    $debug_query = $db->Execute ("UPDATE {$db->prefix}players SET credits=credits+$ship_salvage WHERE player_id=$targetinfo[player_id]");
                    db_op_result($db,$debug_query,__LINE__,__FILE__);

                    $debug_query = $db->Execute ("UPDATE {$db->prefix}ships SET energy=$energy,fighters=fighters-$fighters_lost, armor_pts=armor_pts-$armor_lost, torps=torps-$targettorpnum, ore=ore+$salv_ore, organics=organics+$salv_organics, goods=goods+$salv_goods WHERE ship_id=$ship_id");
                    db_op_result($db,$debug_query,__LINE__,__FILE__);
                }
            }
        }
