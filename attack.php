...
//              playerdeath($db,$playerinfo['player_id'], "LOG_ATTACK_LOSE", "$playerinfo[character_name]|N", 1, $targetinfo['player_id'], $shipinfo['ship_id']);
                ...
                if ($targetarmor > 0)
                {
                    ...
                    $debug_query = $db->Execute ("UPDATE {$db->prefix}ships SET energy=?, fighters=fighters-?, armor_pts=armor_pts-?, torps=torps-?, ore=ore+?, organics=organics+?, goods=goods+? WHERE ship_id=?", array($energy, $fighters_lost, $armor_lost, $targettorpnum, $salv_ore, $salv_organics, $salv_goods, $ship_id));
                    db_op_result($db,$debug_query,__LINE__,__FILE__);
                }
            }
        }
