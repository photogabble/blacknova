    ...
    if(!empty($planetinfo))
    {
      if($planetinfo[owner] == $playerinfo[ship_id] || ($planetinfo[corp] == $playerinfo[team] && $playerinfo[team] <> 0))
      {
		...
		if ($tcrjm > $colonist_limit){
			echo "<font color=\"red\"><B>ATTENTION</B></font><br>You have just killed <font color=\"yellow\">".NUMBER($tcrjm-$colonist_limit)."</font> of your colonists, while trying to dump them on your already over populated planet.<br>\n";
			echo "I hope your pleased with yourself!<br><br>\n";
			$transfer_colonists=0;} 
