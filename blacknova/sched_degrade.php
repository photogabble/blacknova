<?
  ...
  {
     $row = $res->fields;
     $res3 = $db->Execute("SELECT * from $dbtables[players] where player_id = $row[player_id]");
     $sched_playerinfo = $res3->fields;
     $res2 = $db->Execute("SELECT * from $dbtables[planets] where (owner = $row[player_id] or (corp = $sched_playerinfo[team] AND $sched_playerinfo[team] <> 0)) and sector_id = $row[sector_id] and energy > 0"); 
     if($res2->EOF)
     {     
     ...
     else
     {
        ...
        $res4 = $db->Execute("SELECT IFNULL(SUM(energy),0) as energy_available from $dbtables[planets] where (owner = $row[player_id] or (corp = $sched_playerinfo[team] AND $sched_playerinfo[team] <> 0)) and sector_id = $row[sector_id]"); 
