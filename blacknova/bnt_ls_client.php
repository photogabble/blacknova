...
if ($bnt_ls===true) {
     ...
     if($hide_admin_rank == 1)
          $query = " AND $dbtables[ships].email NOT LIKE '$admin_mail' ";
     else
          $query = " ";

     $res = $db->Execute("SELECT COUNT(*) AS x FROM $dbtables[ships] WHERE ship_destroyed='N' and email NOT LIKE '%@furangee'".$query."AND turns_used > 0");
