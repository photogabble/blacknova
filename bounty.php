...
switch ($response) 
{
    case '1': // Display
        echo "<h1>" . $title. "</h1>\n";
        $res5 = $db->Execute("SELECT * FROM {$db->prefix}players,{$db->prefix}bounty WHERE bounty_on = player_id AND bounty_on = $bounty_on");
        $j = 0;
        if ($res5)
        {
            while (!$res5->EOF)
        ...
        else
        {
            ...
            {
                $someres = $db->execute("SELECT character_name FROM {$db->prefix}players WHERE player_id = " . $bounty_details[$j]['placed_by']);
    ...
    case '2': // Cancel
        ...
        $refund = $bty['amount'];
        $debug_query = $db->Execute("UPDATE {$db->prefix}players SET turns=turns-1, turns_used=turns_used+1, credits=credits+$refund WHERE player_id=$playerinfo[player_id]");
    ...
    case '3': // Place
        ...
        $ex = $db->Execute("SELECT * from {$db->prefix}players LEFT JOIN {$db->prefix}ships " .
                           ...
                           "WHERE {$db->prefix}ships.destroyed='N' AND {$db->prefix}players.player_id = $bounty_on");
        if (!$ex)
        {
       ...
       if ($bounty_maxvalue != 0)
       {
            ...
            $previous_bounty = 0;
            $pb = $db->Execute("SELECT SUM(amount) AS totalbounty FROM {$db->prefix}bounty WHERE bounty_on = $bounty_on AND placed_by = $playerinfo[player_id]");
            if ($pb)
            {
                $prev = $pb->fields;
      ...
      db_op_result($db,$debug_query,__LINE__,__FILE__);
      $debug_query = $db->Execute("UPDATE {$db->prefix}players SET turns=turns-1, turns_used=turns_used+1, credits=credits-$amount WHERE player_id=$playerinfo[player_id]");
      db_op_result($db,$debug_query,__LINE__,__FILE__);
      echo "$l_by_placed<br>";
      global $l_global_mmenu;
      echo "<a href=\"main.php\">" . $l_global_mmenu . "</a>";
    ...
    default:
        ...
        $debug_query = $db->Execute("SELECT DISTINCT {$db->prefix}players.* FROM {$db->prefix}ships LEFT JOIN {$db->prefix}players " .
                                    ...
                                    "{$db->prefix}players.player_id != $playerinfo[player_id] ORDER BY character_name ASC");
        db_op_result($db,$debug_query,__LINE__,__FILE__);
        echo '<form name="bntform" action="bounty.php" method="post" onsubmit="document.bntform.submit_button.disabled=true;" accept-charset="utf-8">';
        echo "<table>";
        ...
        else
        {
            ...
            {
                $someres = $db->execute("SELECT character_name FROM {$db->prefix}players WHERE player_id = " . $bounties[$i]['bounty_on']);
                $details = $someres->fields;
                echo "<tr bgcolor=\"$color\">";
