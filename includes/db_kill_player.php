...
//
// File: inclues/db_kill_player.php
...
function db_kill_player($db, $player_id) // Might be able to split out of g_functions..
{
    ...
    $debug_query = $db->Execute("UPDATE {$db->prefix}ships SET destroyed='Y', class=1, hull=0, engines=0, pengines=0, power=0, sensors=0, " .
                                ...
                                "cleared_defenses=' ' WHERE player_id=?", array($start_armor, $start_energy, $start_fighters, $player_id));
    db_op_result($db,$debug_query,__LINE__,__FILE__);

    $debug_query = $db->Execute("UPDATE {$db->prefix}players SET times_dead=times_dead+1 WHERE player_id=?", array($player_id));
    db_op_result($db,$debug_query,__LINE__,__FILE__);

    $debug_query = $db->Execute("UPDATE {$db->prefix}ibank_accounts SET balance='0' WHERE player_id=?", array($player_id));
    db_op_result($db,$debug_query,__LINE__,__FILE__);

    $debug_query = $db->Execute("DELETE FROM {$db->prefix}bounty WHERE placed_by=?", array($player_id));
    db_op_result($db,$debug_query,__LINE__,__FILE__);
    ...
    if ($spy_success_factor)
    {
        ...
        $debug_query = $db->Execute("SELECT ship_id FROM {$db->prefix}ships WHERE player_id=?", array($player_id));
        db_op_result($db,$debug_query,__LINE__,__FILE__);
        $ship_id = $debug_query->fields['ship_id'];
        spy_ship_destroyed($db, $ship_id, 0);

        $debug_query = $db->Execute("DELETE FROM {$db->prefix}spies WHERE owner_id=?", array($player_id));
        db_op_result($db,$debug_query,__LINE__,__FILE__);
    }

    $debug_query = $db->Execute("SELECT DISTINCT sector_id FROM {$db->prefix}planets WHERE owner=? AND base='Y'", array($player_id));
    db_op_result($db,$debug_query,__LINE__,__FILE__);
    $i = 0;
    ...
    $debug_query = $db->Execute("SELECT planet_id FROM {$db->prefix}planets WHERE owner=?", array($player_id));
    while (!$debug_query->EOF && $debug_query)
    {
    ...
    $debug_query = $db->Execute("DELETE FROM {$db->prefix}sector_defense WHERE player_id=?", array($player_id));
    db_op_result($db,$debug_query,__LINE__,__FILE__);

    $debug_query = $db->Execute("SELECT zone_id FROM {$db->prefix}zones WHERE team_zone='N' AND owner=?", array($player_id));
    db_op_result($db,$debug_query,__LINE__,__FILE__);
    $zone = $debug_query->fields;

    $debug_query = $db->Execute("UPDATE {$db->prefix}universe SET zone_id=1 WHERE zone_id=?", array($zone['zone_id']));
    db_op_result($db,$debug_query,__LINE__,__FILE__);

    $debug_query = $db->Execute("SELECT character_name FROM {$db->prefix}players WHERE player_id=?", array($player_id));
    db_op_result($db,$debug_query,__LINE__,__FILE__);
