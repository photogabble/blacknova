...
function sql_log_starvation()
{
    ...
    $debug_query = $db->Execute("SELECT owner, sector_id, ROUND(colonists * ? * ?) AS st_value FROM ".
                 ...
                 "* ? * ? * prod_organics / 100.0 * ?) - " .
                 "(LEAST(colonists, ?) * ? * ? * ?) < 0)", array($starvation_death_rate, $expoprod$line, $colonist_limit, $colonist_production_rate, $organics_prate, $expoprod, $colonist_limit, $colonist_production_rate, $organics_consumption,  $expoprod));
    db_op_result($db,$debug_query,__LINE__,__FILE__);
    return $debug_query;
}
...
function sql_defense_update()
{
    ...
    $debug_query = $db->Execute("UPDATE {$db->prefix}planets SET fighters=fighters + " .
                    ...
                    "$expoprod, torps=torps + (LEAST(colonists, ?) * ?) * ? * " .
...
{
    global $raw_prefix;
    $debug_query = $db->SelectLimit("SELECT {$raw_prefix}accounts.email, {$db->prefix}players.score, " .
                        ...
                        "{$db->prefix}players.player_id={$db->prefix}ships.player_id LEFT JOIN {$raw_prefix}accounts ON {$raw_prefix}accounts.account_id ={$db->prefix}players.account_id WHERE destroyed!='Y' " .
...
function sql_spy_select()
{
    ...
    $debug_query = $db->Execute("SELECT {$db->prefix}spies.*, {$db->prefix}players.character_name, {$db->prefix}ships.name AS " .
                                ...
                                "{$db->prefix}spies.active='Y' AND {$db->prefix}spies.owner_id=? ORDER BY ?", array($playerinfo['player_id'], $by11));
    db_op_result($db,$debug_query,__LINE__,__FILE__);
    return $debug_query;
}
