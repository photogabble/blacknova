...
function playerdeath($db,$corpse_id, $logtype, $loginfo=0, $murdered=0, $murderer_id=0, $ship_id=0)
{
    ...
    $debug_query = $db->Execute("SELECT * FROM {$db->prefix}ships WHERE ship_id=?", array($ship_id));
    ...
    $debug_query = $db->Execute("UPDATE {$db->prefix}players SET credits=0 WHERE player_id=?", array($corpse_id));
    db_op_result($db,$debug_query,__LINE__,__FILE__);
