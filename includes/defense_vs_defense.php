...
//
// File: defense_vs_defense.php
function defense_vs_defense($db, $player_id)
{
    dynamic_loader ($db, "playerlog.php");

    $result1 = $db->Execute("SELECT * FROM {$db->prefix}sector_defense WHERE player_id = $player_id");
    ...
    if ($result1 > 0)
    {
        while (!$result1->EOF)
        {
            ...
            $qty = $row['quantity'];
            $result2 = $db->Execute("SELECT * FROM {$db->prefix}sector_defense WHERE sector_id = $row[sector_id] and player_id != $player_id ORDER BY quantity DESC");
            ...
            if ($result2 > 0)
            {
                while (!$result2->EOF && $qty > 0)
                {
                    ...
                    if ($qty > $cur['quantity'])
                    {
                        $debug_query = $db->Execute("DELETE FROM {$db->prefix}sector_defense WHERE defense_id = $cur[defense_id]");
                        $qty -= $cur['quantity'];
                        db_op_result($db,$debug_query,__LINE__,__FILE__);

                        $debug_query = $db->Execute("UPDATE {$db->prefix}sector_defense SET quantity = $qty WHERE defense_id = $row[defense_id]");
                        db_op_result($db,$debug_query,__LINE__,__FILE__);
                    ...
                    else
                    {
                        ...
                        $debug_query = $db->Execute("UPDATE {$db->prefix}sector_defense SET quantity=quantity - $qty WHERE defense_id = $cur[defense_id]");
                        db_op_result($db,$debug_query,__LINE__,__FILE__);
